...
export default function RMTrainingApp() {
  ...
  const handleRegister = async (e: FormEvent) => {
    ...
    try {
      ...
      alert(`Hubo un error al registrarse. ${error.message}. Intenta de nuevo.`);
    }
  };
        ...
        {isLoggedIn ? (
          <nav className="hidden md:flex items-center gap-4">
            <div className="flex items-center gap-2">
                ...
                <span className="text-white font-medium">Hola, {userData?.name || userData?.username || 'Usuario'}</span>
            </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
              <div className="bg-[#181b2e] p-8 rounded-lg shadow-xl w-80 relative">
                  ...
                  <form onSubmit={handleLogin} className="flex flex-col gap-4">
                      <input type="text" name="username" placeholder="Usuario o Email" value={loginForm.username} onChange={handleLoginChange} required className="p-3 rounded-md bg-[#2a2d3e] text-white placeholder-gray-400" />
